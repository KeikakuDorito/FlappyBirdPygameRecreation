...
same code as round 1 but its a bit different
'''
...
class TopPipe(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.collidepoint(276,0) and count == 1: #Scorekeeping works by everytime it passes the center H4X
            global score
            score += 1
...
#THE FOLLOWING IS WHERE ROUND TWO REALLY MATTERS!

    if count == pipe_delay: #Spawning of the pipes
        mouseloc = pygame.mouse.get_pos() #Get the mouse position
        yloc = mouseloc[1] #make the y value of the mouse the center of entry
        print(yloc) #print so we can track it
        if yloc > 350 or yloc < 75: #If the person controlling the pipes decides to be a game troll and put it somewhere impossible, it just makes it a random value.
            yloc = random.randint(maxheight_pipes,minheight_pipes)
